...
#define LOG_IDENT "KMRE_kylin-kmre-snsorwindow"

void installTranslator()
{
    // 加载自定义翻译文件
    QString tranPath("/usr/share/kylin-kmre-sensorwindow/translations/");
    QTranslator *tran = new QTranslator;
    if (tran->load(QLocale(), QString("kylin-kmre-sensorwindow"), QString("_"), tranPath)) {
...
{
    QString id = QString("kylin-kmre-snsorwindow" + QLatin1String(getenv("DISPLAY")));
    QtSingleApplication a(id, argc, argv);

    if (a.isRunning()) {
        a.sendMessage(QApplication::arguments().length() > 1 ? QApplication::arguments().at(1) : a.applicationFilePath());
        qDebug() << QObject::tr("kylin-kmre-snsorwindow is already running!");
        return EXIT_SUCCESS;
    }
    ...
    QString lockPath = QString(homePath.at(0) + "/.config/kylin-kmre-snsorwindow.lock");
    ...
    if (lockf(fd, F_TLOCK, 0)) {
        syslog(LOG_ERR, "Can't lock single file, kylin-kmre-snsorwindow is already running!");
        exit(0);
    }
