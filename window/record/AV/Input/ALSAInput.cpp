...
static void ALSARecoverAfterOverrun(snd_pcm_t* pcm) {
	syslog(LOG_WARNING, "[ALSARecoverAfterOverrun] Warning: An overrun has occurred, some samples were lost.");
	if(snd_pcm_prepare(pcm) < 0) {
		syslog(LOG_ERR, "[ALSARecoverAfterOverrun] Error: Can't recover device after overrun!");
		throw ALSAException();
	}
	if(snd_pcm_start(pcm) < 0) {
		syslog(LOG_ERR, "[ALSARecoverAfterOverrun] Error: Can't start PCM device after overrun!");
		throw ALSAException();
	}
}
...
ALSAInput::~ALSAInput() {
	...
	if(m_thread.joinable()) {
		syslog(LOG_INFO, "[ALSAInput::~ALSAInput] Stopping input thread ...");
		m_should_stop = true;
		m_thread.join();
	}
...
std::vector<ALSAInput::Source> ALSAInput::GetSourceList() {
	...
	*/
	...
	syslog(LOG_INFO, "[ALSAInput::GetSourceList] Generating source list ...");
	...
	try {
		...
		if(snd_config_update() < 0) {
			syslog(LOG_ERR, "[ALSAInput::GetSourceList] Error: Could not update ALSA configuration!");
			throw ALSAException();
		}
		...
		if(snd_config_search(snd_config, "pcm", &alsa_config_pcms) == 0) {
			...
			snd_config_for_each(i, next, alsa_config_pcms) {
				...
				// add to list
				syslog(LOG_INFO, "[ALSAInput::GetSourceList] Found plugin: [%s] %s", plugin_name.c_str(), plugin_description.c_str());
		...
		while(snd_card_next(&card) == 0 && card >= 0) {
			...
			if(snd_ctl_open(&alsa_ctl, card_name.c_str(), 0) < 0) {
				syslog(LOG_WARNING, "[ALSAInput::GetSourceList] Warning: Could not open sound card %d.", card);
				continue;
			}

			// get card info
			if(snd_ctl_card_info(alsa_ctl, alsa_card_info) < 0) {
				syslog(LOG_WARNING, "[ALSAInput::GetSourceList] Warning: Could not get info for sound card %d.", card);
				continue;
			}
			std::string card_description = snd_ctl_card_info_get_name(alsa_card_info);
			syslog(LOG_INFO, "[ALSAInput::GetSourceList] Found card: [%s] %s", card_name.c_str(), card_description.c_str());
			...
			while(snd_ctl_pcm_next_device(alsa_ctl, &device) == 0 && device >= 0) {
				...
				// add to list
				syslog(LOG_INFO, "[ALSAInput::GetSourceList] Found device: [%s] %s", device_name.c_str(), device_description.c_str());
...
void ALSAInput::Init() {
	...
	try {
		...
		if(snd_pcm_open(&m_alsa_pcm, m_source_name.toUtf8().constData(), SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't open PCM device!");
			throw ALSAException();
		}
		if(snd_pcm_hw_params_any(m_alsa_pcm, alsa_hw_params) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't get PCM hardware parameters!");
			throw ALSAException();
		}

		// set access type
		if(snd_pcm_hw_params_set_access(m_alsa_pcm, alsa_hw_params, SND_PCM_ACCESS_RW_INTERLEAVED) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set access type!");
			throw ALSAException();
		}
		...
		if(snd_pcm_hw_params_set_format_mask(m_alsa_pcm, alsa_hw_params, alsa_format_mask) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set sample format mask!");
			throw ALSAException();
		}
		snd_pcm_format_t sample_format;
		if(snd_pcm_hw_params_set_format_first(m_alsa_pcm, alsa_hw_params, &sample_format) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set sample format!");
			throw ALSAException();
		}
		...
		syslog(LOG_INFO, "[ALSAInput::InputThread] Using sample format %s.", format_str);

		// set sample rate
		unsigned int rate = m_sample_rate;
		if(snd_pcm_hw_params_set_rate_near(m_alsa_pcm, alsa_hw_params, &rate, NULL) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set sample rate!");
			throw ALSAException();
		}
		if(rate != m_sample_rate) {
			syslog(LOG_WARNING, "[ALSAInput::Init] Warning: Sample rate %d is not supported, using %d instead.", m_sample_rate, rate);
			m_sample_rate = rate;
		}
		...
		if(snd_pcm_hw_params_set_channels_near(m_alsa_pcm, alsa_hw_params, &channels) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set channel count!");
			throw ALSAException();
		}
		if(channels != m_channels) {
			syslog(LOG_WARNING, "[ALSAInput::Init] Warning: Channel count %d is not supported, using %d instead.", m_channels, channels);
			m_channels = channels;
		}
		...
		if(snd_pcm_hw_params_set_period_size_near(m_alsa_pcm, alsa_hw_params, &period_size, NULL) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set period size!");
			throw ALSAException();
		}
		if(period_size != m_period_size) {
			syslog(LOG_WARNING, "[ALSAInput::Init] Warning: Period size %d is not supported, using %d instead.", m_period_size, period_size);
		...
		if(snd_pcm_hw_params_set_buffer_size_near(m_alsa_pcm, alsa_hw_params, &buffer_size) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't set buffer size!");
			throw ALSAException();
		}
		if(buffer_size != m_buffer_size) {
			syslog(LOG_WARNING, "[ALSAInput::Init] Warning: Buffer size %d is not supported, using %d instead.", m_buffer_size, buffer_size);
			m_buffer_size = buffer_size;
		}

		// apply parameters
		if(snd_pcm_hw_params(m_alsa_pcm, alsa_hw_params) < 0) {
			syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't apply PCM hardware parameters!");
			throw ALSAException();
		}
	...
	if(snd_pcm_start(m_alsa_pcm) < 0) {
		syslog(LOG_ERR, "[ALSAInput::Init] Error: Can't start PCM device!");
		throw ALSAException();
	}
...
void ALSAInput::InputThread() {
	try {
		...
		syslog(LOG_INFO, "[ALSAInput::InputThread] Input thread started.");
		...
		while(!m_should_stop) {
			...
			if(wait < 0) {
				if(wait == -EPIPE) {
					...
					syslog(LOG_ERR, "[ALSAInput::InputThread] Error: Can't wait for new samples!");
					throw ALSAException();
				}
			} else if(wait == 0) {
				continue;
			...
			if(samples_read < 0) {
				if(samples_read == -EPIPE) {
					...
					syslog(LOG_ERR, "[ALSAInput::InputThread] Error: Can't read samples!");
					throw ALSAException();
				}
		...
		syslog(LOG_INFO, "[ALSAInput::InputThread] Input thread stopped.");

	} catch(const std::exception& e) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[ALSAInput::InputThread] Exception '%s' in input thread.", e.what());
	} catch(...) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[ALSAInput::InputThread] Unknown exception in input thread.");
	}
}
