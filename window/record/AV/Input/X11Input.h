...
class X11Input : public VideoBaseInput {
	Q_OBJECT
...
public:
	...
	void GetCurrentRectangle(unsigned int* x, unsigned int* y, unsigned int* width, unsigned int* height) override;

	// Reads the current size of the stream.
	// This function is thread-safe.
	void GetCurrentSize(unsigned int* width, unsigned int* height) override;
...
private:
	...
	void GetScreenRectangles(std::vector<Rect> &screen_rects);
