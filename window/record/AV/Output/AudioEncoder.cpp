...
void AudioEncoder::PrepareStream(AVStream* stream, AVCodecContext* codec_context, AVCodec* codec, AVDictionary** options, const std::vector<std::pair<QString, QString> >& codec_options,
	...
	if(channels == 0) {
		syslog(LOG_ERR, "[AudioEncoder::PrepareStream] Error: Channel count is zero.");
		throw LibavException();
	}
	if(sample_rate == 0) {
		syslog(LOG_ERR, "[AudioEncoder::PrepareStream] Error: Sample rate is zero.");
		throw LibavException();
	}
	...
	if(codec_context->sample_fmt == AV_SAMPLE_FMT_NONE) {
		syslog(LOG_ERR, "[AudioEncoder::PrepareStream] Error: Encoder requires an unsupported sample format!");
		throw LibavException();
	}
...
bool AudioEncoder::EncodeFrame(AVFrameWrapper* frame) {
	...
	try {
		if(avcodec_send_frame(GetCodecContext(), avframe) < 0) {
			syslog(LOG_ERR, "[AudioEncoder::EncodeFrame] Error: Sending of audio frame failed!");
			throw LibavException();
		}
	} catch(...) {
	...
	for( ; ; ) {
		...
		if(res == 0) { // we have a packet, send the packet to the muxer
			...
			syslog(LOG_ERR, "[AudioEncoder::EncodeFrame] Error: Receiving of audio packet failed!");
			throw LibavException();
		}
	}
	...
	if(avcodec_encode_audio2(GetCodecContext(), packet->GetPacket(), (frame == NULL)? NULL : frame->GetFrame(), &got_packet) < 0) {
		syslog(LOG_ERR, "[AudioEncoder::EncodeFrame] Error: Encoding of audio frame failed!");
		throw LibavException();
	}
