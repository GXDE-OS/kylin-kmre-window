...
int ParseCodecOptionInt(const QString& key, const QString& value, int min, int max, int multiply) {
	...
	if(!parsed) {
		syslog(LOG_ERR, "[ParseCodecOptionInt] Error: Option '%s' could not be parsed!", key.toStdString().c_str());
...
double ParseCodecOptionDouble(const QString& key, const QString& value, double min, double max, double multiply) {
	...
	if(!parsed) {
		syslog(LOG_ERR, "[ParseCodecOptionDouble] Error: Option '%s' could not be parsed!", key.toStdString().c_str());
		throw LibavException();
	}
...
void BaseEncoder::StopThread() {
	...
	if(m_thread.joinable()) {
		syslog(LOG_INFO, "[BaseEncoder::~BaseEncoder] Stopping encoder thread ...");
		m_should_stop = true;
		m_thread.join();
	}
...
void BaseEncoder::Init(AVCodec* codec, AVDictionary** options) {
	...
	if(avcodec_open2(m_codec_context, codec, options) < 0) {
		syslog(LOG_ERR, "[BaseEncoder::Init] Error: Can't open codec!");
		throw LibavException();
	}
	m_codec_opened = true;
	...
	while((t = av_dict_get(*options, "", t, AV_DICT_IGNORE_SUFFIX)) != NULL) {
		syslog(LOG_WARNING, "[BaseEncoder::Init] Warning: Codec option '%s' was not recognised!", t->key);
	}
...
void BaseEncoder::EncoderThread() {
	...
	try {
		...
		syslog(LOG_INFO, "[BaseEncoder::EncoderThread] Encoder thread started.");
		...
		if(!m_should_stop && (m_codec_context->codec->capabilities & AV_CODEC_CAP_DELAY)) {
			syslog(LOG_INFO, "[BaseEncoder::EncoderThread] Flushing encoder ...");
			while(!m_should_stop) {
				if(!EncodeFrame(NULL)) {
					break;
				}
		...
		syslog(LOG_INFO, "[BaseEncoder::EncoderThread] Encoder thread stopped.");

	} catch(const std::exception& e) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[BaseEncoder::EncoderThread] Exception '%s' in encoder thread.", e.what());
	} catch(...) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[BaseEncoder::EncoderThread] Unknown exception in encoder thread.");
	}
