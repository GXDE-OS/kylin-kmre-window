...
Muxer::~Muxer() {
	...
	if(m_started) {
		...
		// stop the encoders
		syslog(LOG_INFO, "[Muxer::~Muxer] Stopping encoders ...");
		...
		if(m_thread.joinable()) {
			syslog(LOG_INFO, "[Muxer::~Muxer] Waiting for muxer thread to stop ...");
			m_thread.join();
		}
...
VideoEncoder* Muxer::AddVideoEncoder(const QString& codec_name, const std::vector<std::pair<QString, QString> >& codec_options,
	...
	try {
		...
		if(avcodec_parameters_from_context(stream->codecpar, codec_context) < 0) {
			syslog(LOG_ERR, "[Muxer::AddVideoEncoder] Error: Can't copy parameters to stream!");
...
#if SSR_USE_AVSTREAM_CODECPAR
		if(avcodec_parameters_from_context(stream->codecpar, codec_context) < 0) {
			syslog(LOG_ERR, "[Muxer::AddAudioEncoder] Error: Can't copy parameters to stream!");
			throw LibavException();
		}
#endif
		av_dict_free(&options);
...
void Muxer::Start() {
	...
	if(avformat_write_header(m_format_context, NULL) != 0) {
		syslog(LOG_ERR, "[Muxer::Start] Error: Can't write header! Don't translate 'header'");
		throw LibavException();
	}
...
void Muxer::Finish() {
	assert(m_started);
	syslog(LOG_INFO, "[Muxer::Finish] Finishing encoders ...");
	for(unsigned int i = 0; i < m_format_context->nb_streams; ++i) {
		assert(m_encoders[i] != NULL);
...
void Muxer::Init() {
	...
	if(format == NULL) {
		syslog(LOG_ERR, "[Muxer::Init] Error: Can't find chosen output format(%s)!", m_container_name.toUtf8().constData());
		throw LibavException();
	}

	syslog(LOG_INFO, "[Muxer::Init] Using format %s (%s).", format->name, format->long_name);
	...
	if(m_format_context == NULL) {
		syslog(LOG_ERR, "[Muxer::Init] Error: Can't allocate format context!");
		throw LibavException();
	}
	...
	if(avio_open(&m_format_context->pb, QFile::encodeName(m_output_file).constData(), AVIO_FLAG_WRITE) < 0) {
		syslog(LOG_ERR, "[Muxer::Init] Error: Can't open output file!");
		throw LibavException();
	}
	...
	if(m_format_context != NULL) {
		...
		if(m_started) {
			if(av_write_trailer(m_format_context) != 0) {
				...
				syslog(LOG_ERR, "[Muxer::Free] Error: Can't write trailer, continuing anyway. Don't translate 'trailer'");
			}
			m_started = false;
		}
...
AVCodec* Muxer::FindCodec(const QString& codec_name) {
	...
	if(codec == NULL) {
		syslog(LOG_ERR, "[Muxer::FindCodec] Error: Can't find codec!");
		throw LibavException();
	}
	return codec;
}
...
AVStream* Muxer::AddStream(AVCodec* codec, AVCodecContext** codec_context) {
	...
	syslog(LOG_INFO, "[Muxer::AddStream] Using codec %s (%s).", codec->name, codec->long_name);
...
#endif
	if(stream == NULL) {
		syslog(LOG_ERR, "[Muxer::AddStream] Error: Can't create new stream!");
		throw LibavException();
	}
	...
	*codec_context = avcodec_alloc_context3(codec);
	if(*codec_context == NULL) {
		syslog(LOG_ERR, "[Muxer::AddStream] Error: Can't create new codec context!");
		throw LibavException();
	}
#else
	...
	*codec_context = stream->codec;
	...
	if(avcodec_get_context_defaults3(*codec_context, codec) < 0) {
		syslog(LOG_ERR, "[Muxer::AddStream] Error: Can't get codec context defaults!");
		throw LibavException();
	}
	...
	if(codec->capabilities & AV_CODEC_CAP_EXPERIMENTAL) {
		syslog(LOG_WARNING, "[Muxer::AddStream] Warning: This codec is considered experimental by libav/ffmpeg.");
...
void Muxer::MuxerThread() {
	try {
		...
		syslog(LOG_INFO, "[Muxer::MuxerThread] Muxer thread started.");
		...
		for( ; ; ) {
			...
			if(av_interleaved_write_frame(m_format_context, packet->GetPacket()) != 0) {
				syslog(LOG_ERR, "[Muxer::MuxerThread] Error: Can't write frame to muxer!");
				throw LibavException();
			}
		...
		syslog(LOG_INFO, "[Muxer::MuxerThread] Muxer thread stopped.");

	} catch(const std::exception& e) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[Muxer::MuxerThread] Exception '%s' in muxer thread.", e.what());
	} catch(...) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[Muxer::MuxerThread] Unknown exception in muxer thread.");
	}
}
