...
OutputManager::~OutputManager() {
	...
	if(m_thread.joinable()) {
		syslog(LOG_INFO, "[OutputManager::~OutputManager] Stopping fragment thread ...");
		m_should_stop = true;
		m_thread.join();
	}
...
void OutputManager::StartFragment() {
	...
	// syslog(LOG_DEBUG, "[OutputManager] container_avname = %s, filename = %s", 
	// 		m_output_settings.container_avname.toStdString().c_str(),
	...
	// syslog(LOG_DEBUG, "[OutputManager] video_codec_avname = %s, video_kbit_rate = %d, "
	// 		"video_width = %d, video_height = %d, video_frame_rate = %d", 
	// 		m_output_settings.video_codec_avname.toStdString().c_str(),
	// 		m_output_settings.video_kbit_rate,
	// 		m_output_settings.video_width, 
	// 		m_output_settings.video_height, 
	// 		m_output_settings.video_frame_rate);
	...
	// syslog(LOG_DEBUG, "[OutputManager] audio_codec_avname = %s, audio_kbit_rate = %d \n"
	// 		"audio_channels = %d, audio_sample_rate = %d", 
	// 		m_output_settings.audio_codec_avname.toStdString().c_str(),
	// 		m_output_settings.audio_kbit_rate,
	// 		m_output_settings.audio_channels, 
...
void OutputManager::FragmentThread() {
	try {
		...
		syslog(LOG_INFO, "[OutputManager::FragmentThread] Fragment thread started.");
		...
		while(!m_should_stop) {
			...
			if(next_fragment) {
				syslog(LOG_INFO, "[OutputManager::FragmentThread] Next fragment ...");
				StopFragment();
				StartFragment();
			} else if(finishing) {
				syslog(LOG_INFO, "[OutputManager::FragmentThread] Finishing ...");
				StopFragment();
				break;
			} else {
				usleep(200000);
		...
		syslog(LOG_INFO, "[OutputManager::FragmentThread] Fragment thread stopped.");

	} catch(const std::exception& e) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[OutputManager::FragmentThread] Exception '%s' in fragment thread.", e.what());
	} catch(...) {
		m_error_occurred = true;
		syslog(LOG_ERR, "[OutputManager::FragmentThread] Unknown exception in fragment thread.");
	}
}
