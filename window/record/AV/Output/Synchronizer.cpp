...
Synchronizer::~Synchronizer() {
	...
	if(m_thread.joinable()) {
		syslog(LOG_INFO, "[Synchronizer::~Synchronizer] Stopping synchronizer thread ...");
		m_should_stop = true;
		m_thread.join();
	}
...
void Synchronizer::ReadVideoFrame(unsigned int width, unsigned int height, const uint8_t* data, int stride, AVPixelFormat format, int colorspace, int64_t timestamp) {
	...
	if(timestamp < videolock->m_last_timestamp) {
		if(timestamp < videolock->m_last_timestamp - 10000)
			syslog(LOG_WARNING, "[Synchronizer::ReadVideoFrame] Warning: Received video frame with non-monotonic timestamp.");
	...
	if(lock->m_video_buffer.size() >= MAX_VIDEO_FRAMES_BUFFERED) {
		if(lock->m_segment_audio_started) {
			if(lock->m_warn_drop_video) {
				...
				syslog(LOG_WARNING, "[Synchronizer::ReadVideoFrame] Warning: Video buffer overflow, some frames will be lost. The audio input seems to be too slow.");
			}
			return;
		} else {
...
void Synchronizer::ReadAudioSamples(unsigned int channels, unsigned int sample_rate, AVSampleFormat format, unsigned int sample_count, const uint8_t* data, int64_t timestamp) {
	...
	if(timestamp < audiolock->m_last_timestamp) {
		if(timestamp < audiolock->m_last_timestamp - 10000)
			syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Received audio samples with non-monotonic timestamp.");
	...
	if(current_drift > DRIFT_ERROR_THRESHOLD && !audiolock->m_drop_samples) {
		...
		syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Too many audio samples, dropping samples to keep the audio in sync with the video.");
	}
	...
	if(current_drift < -DRIFT_ERROR_THRESHOLD && !audiolock->m_insert_samples) {
		...
		syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Not enough audio samples, inserting silence to keep the audio in sync with the video.");
	}
	...
	if(audiolock->m_average_drift < -0.02 && audiolock->m_warn_desync) {
		...
		syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Audio input is more than 2% too slow!");
	}
	if(audiolock->m_average_drift > 0.02 && audiolock->m_warn_desync) {
		audiolock->m_warn_desync = false;
		syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Audio input is more than 2% too fast!");
	}
	...
	if(lock->m_audio_buffer.GetSize() / m_output_format->m_audio_channels >= MAX_AUDIO_SAMPLES_BUFFERED) {
		if(lock->m_segment_video_started) {
			syslog(LOG_WARNING, "[Synchronizer::ReadAudioSamples] Warning: Audio buffer overflow, starting new segment to keep the audio in sync with the video "
...
void Synchronizer::ReadAudioHole() {
	...
	if(audiolock->m_first_timestamp != (int64_t) AV_NOPTS_VALUE) {
		...
		if(!audiolock->m_drop_samples || !audiolock->m_insert_samples) {
			syslog(LOG_WARNING, "[Synchronizer::ReadAudioHole] Warning: Received hole in audio stream, inserting silence to keep the audio in sync with the video.");
...
void Synchronizer::FlushVideoBuffer(Synchronizer::SharedData* lock, int64_t segment_start_time, int64_t segment_stop_time) {
	...
	for( ; ; ) {
		...
		while(lock->m_segment_video_accumulated_delay >= delay_time_per_frame && lock->m_video_pts < segment_stop_video_pts) {
			...
			lock->m_video_pts += 1;
			//syslog(LOG_INFO, "[Synchronizer::FlushVideoBuffer] Delay [%d] acc %d.", lock->m_video_pts - 1, lock->m_segment_video_accumulated_delay);
		}
		...
		if(lock->m_last_video_frame_data != NULL) {
			while(lock->m_video_pts + m_max_frames_skipped < std::min(next_pts, segment_stop_video_pts)) {
				...
				//syslog(LOG_INFO, "[Synchronizer::FlushVideoBuffer] Encoded video frame [%d] (duplicate) acc %d.", duplicate_frame->GetFrame()->pts, lock->m_segment_video_accumulated_delay);
		...
		if(frame->GetFrame()->pts < lock->m_video_pts) {
			//syslog(LOG_INFO, "[Synchronizer::FlushVideoBuffer] Dropped video frame [%d] acc %d.", frame->GetFrame()->pts, lock->m_segment_video_accumulated_delay);
			continue;
		}
		...
		//syslog(LOG_INFO, "[Synchronizer::FlushBuffers] Encoded video frame [%d].", frame->GetFrame()->pts);
	...
	if(lock->m_audio_buffer.GetSize() > 0) {
		...
		while(samples_left > 0) {
			...
			if(lock->m_partial_audio_frame_samples == m_output_format->m_audio_frame_size) {
				...
				//syslog(LOG_INFO, "[Synchronizer::FlushAudioBuffer] Encoded audio frame [%d].", lock->m_partial_audio_frame->pts);
...
void Synchronizer::SynchronizerThread() {
	try {
		...
		syslog(LOG_INFO, "[Synchronizer::SynchronizerThread] Synchronizer thread started.");
		...
		m_error_occurred = true;
		syslog(LOG_ERR, "[Synchronizer::SynchronizerThread] Unknown exception in synchronizer thread.");
	}
}
