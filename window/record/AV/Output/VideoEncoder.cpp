...
void VideoEncoder::PrepareStream(AVStream* stream, AVCodecContext* codec_context, AVCodec* codec, AVDictionary** options, const std::vector<std::pair<QString, QString> >& codec_options,
	...
	if(width == 0 || height == 0) {
		syslog(LOG_ERR, "[VideoEncoder::PrepareStream] Error: Width or height is zero!");
		throw LibavException();
	}
	if(width > SSR_MAX_IMAGE_SIZE || height > SSR_MAX_IMAGE_SIZE) {
		syslog(LOG_ERR, "[VideoEncoder::PrepareStream] Error: Width or height is too large, the maximum width and height is %d!", SSR_MAX_IMAGE_SIZE);
		throw LibavException();
	}
	if(width % 2 != 0 || height % 2 != 0) {
		syslog(LOG_ERR, "[VideoEncoder::PrepareStream] Error: Width or height is not an even number!");
		throw LibavException();
	}
	if(frame_rate == 0) {
		syslog(LOG_ERR, "[VideoEncoder::PrepareStream] Error: Frame rate is zero!");
		throw LibavException();
	}
	...
	for(unsigned int i = 0; i < SUPPORTED_PIXEL_FORMATS.size(); ++i) {
		...
		if(!AVCodecSupportsPixelFormat(codec, SUPPORTED_PIXEL_FORMATS[i].m_format))
			continue;
		syslog(LOG_INFO, "[VideoEncoder::PrepareStream] Using pixel format %s.", 
							SUPPORTED_PIXEL_FORMATS[i].m_name.toStdString().c_str());
	...
	if(codec_context->pix_fmt == AV_PIX_FMT_NONE) {
		syslog(LOG_ERR, "[VideoEncoder::PrepareStream] Error: The pixel format is not supported by the codec!");
		throw LibavException();
	}
...
bool VideoEncoder::EncodeFrame(AVFrameWrapper* frame) {
	...
	try {
		if(avcodec_send_frame(GetCodecContext(), avframe) < 0) {
			syslog(LOG_ERR, "[VideoEncoder::EncodeFrame] Error: Sending of video frame failed!");
			throw LibavException();
		}
	} catch(...) {
	...
	for( ; ; ) {
		...
		if(res == 0) { // we have a packet, send the packet to the muxer
			...
			syslog(LOG_ERR, "[VideoEncoder::EncodeFrame] Error: Receiving of video packet failed!");
			throw LibavException();
		}
	}
	...
	if(avcodec_encode_video2(GetCodecContext(), packet->GetPacket(), (frame == NULL)? NULL : frame->GetFrame(), &got_packet) < 0) {
		syslog(LOG_ERR, "[VideoEncoder::EncodeFrame] Error: Encoding of video frame failed!");
		throw LibavException();
	}
