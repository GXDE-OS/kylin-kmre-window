...
void PopupTip::popup(const QPoint &pos, int showTime)
{
    ...
    raise();
    show();
    //syslog(LOG_DEBUG, "[PopupTip] show tip!\n");

    if (showTime > 0) {
        QTimer::singleShot(showTime, [=] {
            //syslog(LOG_DEBUG, "[PopupTip] time out, hide tip!\n");
            hide();
        });
    }
}
...
void PopupTip::keyPressEvent(QKeyEvent* event)
{
    if (event->key() == Qt::Key_Escape) {
        //syslog(LOG_DEBUG, "[PopupTip] keyPressEvent\n");
        this->hide();
    }
...
{
    //syslog(LOG_DEBUG, "[PopupTip] mousePressEvent\n");
    this->hide();
