...
JoystickAutoKey::JoystickAutoKey(KmreWindow *window) :
    JoystickSingleBaseKey(window, AUTOMETHOD)
{
    ...
    mInputEdit->setStyleSheet("QLineEdit{background:transparent;border-image:url(:/new/res/auto_hover.svg);color:rgba(255,255,255,1);}");

    this->installEventFilter(this);
    mInputEdit->installEventFilter(this);
}
void JoystickAutoKey::updateSize(bool update_panel)
{
...
void JoystickAutoKey::enableEdit(bool enable)
{
    if(enable){
        mInputEdit->setStyleSheet("QLineEdit{background:transparent;border-image:url(:/new/res/auto_hover.svg);color:rgba(255,255,255,1);}");
    }else{
        mInputEdit->setStyleSheet("QLineEdit{background:transparent;border-image:url(:/new/res/auto_normal.svg);color:rgba(255,255,255,1);}");
    }
...
void JoystickAutoKey::getTranslateCenterPos(QPoint pos){
